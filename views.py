...
#     graph_model.add(GraphCNN(16, 1, graph_conv_filters,
#                              input_shape=(X_train.shape[1],),
#                              activation='elu',
#                              kernel_regularizer=l2(0.001)))

#     graph_model.add(GraphCNN(8, 1, graph_conv_filters,
#                              input_shape=(X_train.shape[1],),
#                              activation='elu',
#                              kernel_regularizer=l2(0.001)))

#     graph_model.add(GraphCNN(1, 1, graph_conv_filters,
#                              input_shape=(X_train.shape[1],),
#                              activation='elu',
#                              kernel_regularizer=l2(0.001)))
...
#     # ✅ FIXED PART
#     if os.path.exists("model/gnn_weights.h5"):
...
def getModel():
    global X_train, y_train, graph, session
...
def PredictAction(request):
    ...
    if request.method == 'POST':
        ...
        with graph.as_default():
            ...
            data = [[
                int(account_age), gender,
                int(user_age), link_desc,
                int(status_count), int(friend_count),
                internet, task, wifi
            ]]

            data = pd.DataFrame(data, columns=[
                'Account_Age','Gender','User_Age','Link_Desc',
                'Status_Count','Friend_Count','internet','gettask','changewifis'
            ])
        ...
        saveProfile(account_age, gender, user_age, link_desc,
                    status_count, friend_count, internet, task, wifi,
                    labels[predict])

        status = "Genuine Profile Detected" if predict == 1 else "Fake Profile Detected"

        return render(request, 'Predict.html', {
            'data': "GNN Prediction Result = " + status
        })
